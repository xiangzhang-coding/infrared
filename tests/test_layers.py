"""Unit tests for the T0 Qwen2 building blocks (torch-only, no network/model).

These pin the math that the Seam-A parity gate later checks end-to-end: RMSNorm's
fp32 path, RoPE, GQA head expansion, the hand-written causal attention (vs
torch SDPA), SwiGLU, and the sampler. Fast and hardware-free — run everywhere
torch is installed.
"""
...
def test_causal_attention_prefill_matches_sdpa() -> None:
    torch.manual_seed(0)
    seq, heads, dim = 6, 4, 8
    q, k, v = (torch.randn(seq, heads, dim) for _ in range(3))
    pos = torch.arange(seq)
    got = causal_attention(q, k, v, pos, pos, scale=dim**-0.5)
    # SDPA treats leading dim as heads: [H, S, D].
...
def test_causal_attention_decode_attends_all_history() -> None:
    torch.manual_seed(0)
    total, heads, dim = 5, 4, 8
    k, v = torch.randn(total, heads, dim), torch.randn(total, heads, dim)
    q = torch.randn(1, heads, dim)  # single decode query at position total-1
    got = causal_attention(
        q, k, v, torch.tensor([total - 1]), torch.arange(total), scale=dim**-0.5
    )
    ref = F.scaled_dot_product_attention(
